...
public class TwoWheeler implements Vehicle{
    ...
    public TwoWheeler(Integer customerID, HashMap<Integer, Slot> customerBooking, Boolean booked,ParkingSpace parkingSpace) {
    ...
    public boolean getAvailability(Integer slotID, Integer startTime, Integer endTime) {
        if(customerBooking.get(slotID)!=null && customerBooking.get(slotID).isFilled()) {
    ...
    public void processBooking(Integer slotID, Integer startTime, Integer endTime) {
        if(getAvailability(slotID,startTime,endTime)) {
            this.booked=true;
            parkingSpace.checkIn(startTime);
            parkingSpace.checkOut(endTime);
            Slot bookedSlot=new Slot(slotID,true,new Pair<>(startTime,endTime));
            customerBooking.put(slotID,bookedSlot);
