...
public class Host  extends Calendar{
    ...
    public void scheduled(Date startDate,Date endDate) {
        int start=startDate.getDate(),end= endDate.getDate();
        for(int i=start;i<=end;i++) {
            Date newDate=new Date(dateList.get(i).getYear(), dateList.get(i).getMonth(), dateList.get(i).getDay());
            availability.put(newDate, false);
        }
        System.out.println("meeting scheduled for : ["+startDate+" : "+endDate+"]");
    }
    public void canceled(Date startDate,Date endDate) {
        if(!availability.get(startDate) && !availability.get(endDate)) {
            ...
            System.out.println("meeting cancelled for : ["+startDate+" : "+endDate+"]");
        }
        else {
